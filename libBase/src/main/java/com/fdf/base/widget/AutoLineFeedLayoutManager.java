...
public class AutoLineFeedLayoutManager extends RecyclerView.LayoutManager {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        for (int i = 0; i < getItemCount(); i++) {
            ...
            int width = getDecoratedMeasuredWidth(view) + params.getMarginStart() + params.getMarginEnd();
            ...
            if (curLineWidth <= getWidth()) {//如果累加的宽度小于等于RecyclerView的宽度，不需要换行
                layoutDecorated(view, curLineWidth - width + params.getMarginStart(), curLineTop + params.topMargin, curLineWidth - params.getMarginEnd(), curLineTop + height - params.bottomMargin);//布局item的真实位置
                ...
                layoutDecorated(view, params.getMarginStart(), curLineTop + params.topMargin, width - params.getMarginEnd(), curLineTop + height - params.bottomMargin);
                lastLineMaxHeight = height;
            }
        }
    }
    ...
    public boolean isRtl() {
        return TextUtilsCompat.getLayoutDirectionFromLocale(BaseApplication.instance.getResources().getConfiguration().locale) == ViewCompat.LAYOUT_DIRECTION_RTL;
    }
