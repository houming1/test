...
class PopDateTimePicker constructor(context: Context) : BottomPopupView(context) {
    ...
    private lateinit var bind: PopDateTimePickerBinding
    var type = 1
    ...
    fun setTime() {
        if (type == 1) {
            bind.startTime.text = "$currDate $currTime"
        } else {
            bind.endTime.text = "$currDate $currTime"
        }
        Log.e("hm----startTime",bind.startTime.text.toString())
        Log.e("hm----endTime",bind.endTime.text.toString())
